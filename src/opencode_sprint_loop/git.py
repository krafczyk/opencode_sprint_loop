...
def _run(path: Path, *arguments: str, allow_failure: bool = False) -> str:
    ...
    try:
        ...
        raise ControllerError(
            "root_not_git_worktree", f"Cannot inspect Git repository at {path}"
        ) from error
...
def inspect_worktree(root: Path, *, expected_root: Path, error_code: str) -> GitRepository:
    ...
    if resolved_top_level != expected_root:
        raise ControllerError(
            error_code, f"Expected Git worktree root {expected_root}, found {resolved_top_level}"
        )
    ...
    git_dir = (
        ...
        if not Path(git_dir_value).is_absolute()
        else Path(git_dir_value).resolve()
    )
...
def is_tracked_path(root: Path, path: Path) -> bool:
    ...
    try:
        ...
        raise ControllerError(
            "internal_error", f"Path is outside the sprint root: {path}"
        ) from error
    return bool(
        _run(root, "ls-files", "--error-unmatch", "--", relative, allow_failure=True).strip()
    )
...
def _ensure_clean(
    ...
    for record in records:
        if (
            record.startswith("? ")
            and allowed_untracked is not None
            and record[2:] in allowed_untracked
        ):
            continue
        if record.startswith("! ") and allowed_untracked is not None:
            ignored_path = record[2:].rstrip("/")
            if record[2:] in allowed_untracked or _ignored_tree_contains_only(
                repository.root, ignored_path, allowed_untracked
            ):
                continue
    ...
    for current, directories, files in os.walk(tree, followlinks=False, onerror=errors.append):
        for directory in directories:
            ...
            if candidate.is_symlink() or not any(
                path.startswith(f"{candidate_relative}/") for path in allowed
            ):
                return False
...
def _ensure_no_operation(repository: GitRepository, label: str) -> None:
    ...
    for marker, operation in markers.items():
        if (repository.git_dir / marker).exists():
            raise ControllerError(
                "git_operation_in_progress", f"{label} has an active {operation}: {repository.root}"
            )
    if (repository.git_dir / "rebase-apply").exists() or (
        repository.git_dir / "rebase-merge"
    ).exists():
        raise ControllerError(
            "git_operation_in_progress", f"{label} has an active rebase: {repository.root}"
        )
...
def _ensure_submodule(root: GitRepository, config: SprintConfig) -> GitRepository:
    ...
    if len(fields) < 3 or fields[0] != "160000":
        raise ControllerError(
            "invalid_submodule", f"Managed repository is not a tracked gitlink: {repository.path}"
        )
    gitlink_sha = fields[1]
    modules = root.root / ".gitmodules"
    if modules.is_symlink() or not modules.is_file():
        raise ControllerError(
            "invalid_submodule", f"Missing .gitmodules for managed repository: {repository.path}"
        )
    registered = _run(
        root.root,
        "config",
    ...
    if len(matching_names) != 1:
        raise ControllerError(
            "invalid_submodule",
            f"Managed repository is not registered in .gitmodules: {repository.path}",
        )
    if not repository.path.exists():
        raise ControllerError(
            "uninitialized_submodule",
            f"Managed submodule is not initialized; initialize it before running: {repository.path}",
        )
    try:
        managed = inspect_worktree(
            repository.path, expected_root=repository.path, error_code="uninitialized_submodule"
        )
        ...
        raise ControllerError(
            "uninitialized_submodule",
            f"Managed submodule is not a usable worktree: {repository.path}",
        ) from error
    ...
    if embedded_git and expected_git_dir.exists():
        raise ControllerError(
            "uninitialized_submodule",
            f"Managed repository is not the registered submodule; initialize it before running: {repository.path}",
        )
...
def validate_preflight(
    ...
    *,
    require_clean: bool,
    allowed_root_untracked: set[str] | None = None,
) -> GitRepository:
    ...
    if agents_path.is_symlink() or not agents_path.is_file():
        raise ControllerError(
            "missing_required_file", f"Missing root AGENTS.md: {root / 'AGENTS.md'}"
        )
    ...
    if not branch:
        raise ControllerError(
            "wrong_branch",
            f"Managed repository {managed.root} is detached; check out expected branch {config.repositories[0].branch}",
        )
    repository = config.repositories[0]
    if branch != repository.branch:
        raise ControllerError(
            "wrong_branch",
            f"Managed repository {managed.root} is on branch {branch}; expected {repository.branch}. Check out the configured branch before running",
        )
    remotes = _run(managed.root, "remote").splitlines()
    if repository.remote not in remotes:
        raise ControllerError(
            "missing_remote",
            f"Managed repository {managed.root} has no configured remote {repository.remote}; add it before running",
        )
    if require_clean:
